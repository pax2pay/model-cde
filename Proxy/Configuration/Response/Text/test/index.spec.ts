...
describe("@pax2pay/model.Proxy.Configuration.Response.Text", () => {
	it("extract", async () => {
		const data = dataset["generic"]
		const config = configurations["expires"]
		expect(model.Proxy.Configuration.Response.Text.is(config)).toBeTruthy()

		const card = model.Proxy.Configuration.Response.Text.extract(config, data)
	...
	it("process regular", async () => {
		...
		const result = model.Proxy.Configuration.Response.Text.process(config, token, data)
		const expected =
