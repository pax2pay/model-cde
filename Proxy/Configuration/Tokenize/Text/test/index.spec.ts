...
describe("@pax2pay/model.Proxy.Configuration.Tokenize.Text", () => {
	it("extract", async () => {
		...
		expect(model.Proxy.Configuration.Tokenize.Text.is(config)).toBeTruthy()

		const card = model.Proxy.Configuration.Tokenize.Text.extract(config, data)
	...
	it("process regular", async () => {
		...
		const result = model.Proxy.Configuration.Tokenize.Text.process(config, token, data)
		const expected =
